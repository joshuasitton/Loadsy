...
most-asked models first, and for each the years people drive – the years say which
generation's figures to find. A run of "Other" for one make is a model to add to
`VEHICLE_MODELS` in `src/domain/vehicleRequest.ts`. Until there are counts, use the order below.
